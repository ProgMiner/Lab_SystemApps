...
static tpool_work_t *tpool_work_create(thread_func_t func, void *arg, bool await)
{
    tpool_work_t *work;
    ...
    if (await) {
        pthread_mutex_init(&(work->result_mutex), NULL);
...
static void tpool_work_destroy(tpool_work_t *work)
{
    ...
    if (work->await) {
        pthread_mutex_destroy(&(work->result_mutex));
    }
...
tpool_work_t *tpool_add_work(tpool_t *tm, thread_func_t func, void *arg, bool await)
{
    ...
    work = tpool_work_create(func, arg, await);
    if (work == NULL)
        return NULL;
